...
class Server(multiprocessing.Process):
    ...
    def listen_for_clients(self):
        ...
        while True:
            ...
            if data:
                message = data.decode('utf-8')
                splitted = message.split("_")  # splitted[0] = "register", splitted[1] = "CHAT"
                if splitted[0] == 'register':
                    # Den check auf splitted[1] kannst du weglassen oder überschreiben
                    # group_name = splitted[1].upper()  # entfällt
                    group_name = "CHAT"  # Wir überschreiben es fest
                    self.register_client(group_name, addr)
    ...
    def distribute_chat_message(self, message, addr):
        ...
        receiver_list = []
        for key in self.local_clients_cache:
            if group in key:  # ist fast immer true, wenn wir nur "CHAT" haben
                # Nur nicht an den Sender selbst schicken:
                if self.local_clients_cache[key][0] != addr[0]:
