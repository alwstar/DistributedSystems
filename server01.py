...
class Server(multiprocessing.Process):
    ...
    def register_client(self, group, client_addr):
        ...
        print(f"{self.server_id}: Registering client to GLOBAL chat")
        server_addr = self.server_address
        self.send_reply_to_client(server_addr, client_addr)
        
        client_count = self.filter_clients("GLOBAL")
        self.client_cache_key_offset = client_count + 1
        client_cache_key = "GLOBAL" + str(self.client_cache_key_offset)
    ...
    def distribute_chat_message(self, message, addr):
        try:
            ...
            # Alle Clients außer dem Sender in die Empfängerliste aufnehmen
            for key in self.local_clients_cache:
                if addr[0] != self.local_clients_cache[key][0]:
                    receiver_list.append(self.local_clients_cache[key][0])
            
            # Nachricht mit Absendername verteilen
            formatted_message = f"{sender_name}: {msg_content}"
            self.send_chat_message_to_clients(formatted_message.encode('utf-8'), receiver_list)
            
        except json.JSONDecodeError:
            print(f"{self.server_id}: Error decoding message")
